...
/**
 * The ack carries from, class, id and t, and nothing else. The server id lands
 * separately, in the <status> stanza the server sends back to the publisher --
 * WASmaxInStatusDeliverFromNewsletterMixin reads from, server_id, t and
 * is_sender off exactly that tag. Reacting to or revoking your own status needs
 * that id, so wait a moment for the echo rather than returning undefined.
 */
export const waitForNewsletterStatusServerId = (sock, { jid, messageId, timeoutMs = 8000 }) => new Promise(resolve => {
    let done = false
    const finish = value => {
        if (done) return
        done = true
...
export const makeNewsletterStatusSender = (sock, config) => async (jid, content, options = {}) => {
    ...
    const echo = resolveServerId === false
        ? null
        : waitForNewsletterStatusServerId(sock, {
            jid,
            messageId: fullMsg.key.id,
            timeoutMs: serverIdTimeoutMs ?? NEWSLETTER_STATUS_SERVER_ID_TIMEOUT_MS
        })
