...
const assertMusicHost = (value, label) => {
    ...
    if (!isMusicHostAllowed(value)) {
        throw new TypeError(
            `${label} must be hosted on ${MUSIC_ALLOWED_HOSTS.join(', ')} — the client drops anything else `
            + 'without drawing the bubble, so upload the file first and use the url that comes back'
        )
    }
    return value
}
