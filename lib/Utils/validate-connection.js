...
const getUserAgent = (config) => {
    const isAndroid = config.browser[1].toLocaleLowerCase().includes('android');
    ...
    return {
        ...
        osVersion: info.osVersion ?? (isAndroid ? '14' : '0.1'),
        device: info.device ?? (isAndroid ? config.browser[0] || 'Pixel' : 'Desktop'),
        ...(info.manufacturer || isAndroid ? { manufacturer: info.manufacturer ?? 'Google' } : {}),
        osBuildNumber: info.osBuildNumber ?? (isAndroid ? '14' : '0.1'),
        localeLanguageIso6391: info.localeLanguage ?? 'en',
        mnc: info.mnc ?? '000',
        mcc: info.mcc ?? '000',
