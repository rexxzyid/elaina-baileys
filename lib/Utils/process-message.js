...
const resolveMessageEditAuthor = async (key, meId, lidMapping) => {
    if (!key) {
        return '';
    }
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    if (protocolMsg) {
        ...
        const readStubJson = (index) => {
            const raw = message.messageStubParameters?.[index];
            ...
            try {
                ...
                logger?.warn?.({ jid, stubType: message.messageStubType, raw }, 'group stub parameter is not JSON, skipping it');
                return undefined;
            }
        };
        const readStubParticipants = () => {
            const raw = message.messageStubParameters;
            if (!Array.isArray(raw)) {
                logger?.warn?.({ jid, stubType: message.messageStubType }, 'group stub carries no parameters, treating it as an empty participant list');
                return [];
            }
            return raw.map((_, index) => readStubJson(index)).filter(entry => entry !== undefined);
        };
        switch (message.messageStubType) {
            case WAMessageStubType.GROUP_PARTICIPANT_CHANGE_NUMBER:
                participants = readStubParticipants();
            ...
            case WAMessageStubType.GROUP_MEMBERSHIP_JOIN_APPROVAL_REQUEST_NON_ADMIN_ADD:
                const participant = readStubJson(0);
