...
export const HTML_APP_BYTE_LIMIT = 200 * 1024;
...
export const checkHtmlApp = (html, { height, maxBytes = HTML_APP_BYTE_LIMIT } = {}) => {
    ...
    if (bytes > maxBytes) {
        problems.push(`the page is ${Math.round(bytes / 1024)}KB, over the ${Math.round(maxBytes / 1024)}KB budget — the whole app travels inside the message`);
    }
    ...
    if (/RTCPeerConnection/.test(source)) {
        transports.push('RTCPeerConnection');
    }
    if (transports.length) {
        warnings.push(`${transports.join(' and ')} ${transports.length === 1 ? 'rides' : 'ride'} a transport the HTTP measurements never covered — untested here rather than known-dead, so probe it on a device before building on it`);
    }
    ...
    if (height === undefined) {
        ...
        if (!pinned) {
            warnings.push('no fixed height — pass height to sendHtmlApp, or the host and the page measure each other and the card shudders');
        }
    }
