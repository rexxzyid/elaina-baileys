...
export const checkHtmlApp = (html, { height, maxBytes = HTML_APP_BYTE_LIMIT } = {}) => {
    ...
    if (/\bfetch\s*\(|XMLHttpRequest|navigator\.sendBeacon|new\s+WebSocket|new\s+EventSource/.test(source)) {
        problems.push('a network call — fetch, XHR, WebSocket and EventSource all fail in this WebView, and no securitypolicyviolation fires to tell the page why');
    }
