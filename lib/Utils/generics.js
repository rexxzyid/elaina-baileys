...
const baileysVersion = [2, 3000, 1046401417];
...
export const generateMessageIDHex = (length = 32, prefix = '') => {
    ...
    if (head.length > total) {
        throw new TypeError('prefix is longer than the requested length');
    }
    if (head && !/^[0-9A-F]*$/.test(head)) {
        throw new TypeError('prefix must be hex characters only');
    }
    const rest = total - head.length;
    return head + randomBytes(Math.ceil(rest / 2)).toString('hex').toUpperCase().slice(0, rest);
};
...
export const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/rexxhayanasi/elaina-baileys/main/lib/Defaults/index.js';
    try {
