...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'raw')) {
        ...
        const { audio, artwork, title, author, url, largeThumbnail = true, thumbnailWidth = 320, ...rest } = message.song;
        if (!audio) {
            throw new Boom('song needs audio', { statusCode: 400 });
        }
        m = await prepareWAMessageMedia({ audio, mimetype: 'audio/mpeg', ...rest }, options);
