...
export const prepareLinkThumbnail = async (image, options, thumbnailWidth = 640) => {
    if (typeof options?.upload !== 'function') {
        throw new Boom('a link thumbnail has to be uploaded, so this needs an upload function', { statusCode: 400 });
    }
    ...
    let height;
    try {
        const large = await extractImageThumb(source, thumbnailWidth);
        full = large.buffer;
        const scale = large.original.width ? thumbnailWidth / large.original.width : 1;
        width = Math.round(Math.min(large.original.width ?? thumbnailWidth, thumbnailWidth));
        height = large.original.height ? Math.round(large.original.height * Math.min(scale, 1)) : undefined;
    }
    catch (error) {
        options.logger?.warn?.({ trace: error?.message }, 'could not downscale the link thumbnail, uploading it whole');
    }
    let jpegThumbnail;
    try {
        jpegThumbnail = (await extractImageThumb(source, 192)).buffer;
    }
    catch {
        jpegThumbnail = full;
    }
    const upload = await encryptedStream(full, 'thumbnail-link', {
        logger: options.logger,
        opts: options.options
    });
    let result;
    try {
        result = await options.upload(upload.encFilePath, {
        ...
        fs.unlink(upload.encFilePath).catch(() => options.logger?.warn?.('failed to remove tmp file'));
    }
    return {
        jpegThumbnail,
...
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = '📦 Sticker Pack', publisher = 'GitHub: rexxhayanasi', description = '🏷️ rexxhayanasi/elaina-baileys' } = message;
...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'raw')) {
        ...
        if (image) {
            const thumbnails = await prepareLinkThumbnail(image, options, thumbnailWidth);
        ...
        if (urlInfo?.image && !urlInfo.highQualityThumbnail) {
            urlInfo = { ...urlInfo, ...(await prepareLinkThumbnail(urlInfo.image, options, urlInfo.thumbnailWidth)) };
        }
        if (urlInfo) {
