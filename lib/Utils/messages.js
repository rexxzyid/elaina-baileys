...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'raw')) {
        ...
        const pollCreationMessage = {
            ...
            // Left undefined rather than false when the caller did not ask for
            // them. WhatsApp omits these fields entirely on a plain poll, and an
            // explicit false is a different set of bytes on the wire even though
        ...
        if (message.poll.toAnnouncementGroup) {
            ...
            if (message.poll.pollType === 1) {
                ...
                /**
                 * Every version carries the same PollCreationMessage, so V3 holds
                 * the end time and hidden-voter flags just as V6 does, and the
                 * receiver reads them from whichever version arrived. The option
                 * images only attach on V3, so a photo poll stays there even when
                 * settings are set.
                 */
                m.pollCreationMessageV3 = pollCreationMessage;
            }
            else if (usesExtendedPollSettings) {
                // WhatsApp switches the message version on the settings, not on
                // the option count: a poll carrying an end time, hidden voters or
                // add-option goes out as V6, and those fields are ignored on the
                // older versions.
