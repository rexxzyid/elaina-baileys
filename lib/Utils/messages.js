...
const SUPPORTED_NATIVE_FLOW_NAMES = new Set([
    'quick_reply', 'cta_call', 'cta_url', 'cta_catalog', 'catalog_message', 'cta_copy_code',
    'galaxy_message', 'order_status', 'payment_reminder', 'booking_confirmation',
    'payment_request', 'api_signup', 'inapp_signup', 'cta_app', 'form_message'
]);

const NATIVE_FLOW_NAME_BY_BUTTON = {
...
export const QUICK_REPLY_BUTTON_LIMIT = 10;
export const NATIVE_FLOW_BUTTON_LIMIT = 3;
...
export const nativeFlowButtonsViolateConstraints = (buttons = []) => {
    ...
    const limit = firstIsQuickReply ? QUICK_REPLY_BUTTON_LIMIT : NATIVE_FLOW_BUTTON_LIMIT;
    if (buttons.length > limit) {
        return true;
    }
    return !buttons.slice(1).every(button => {
        const mapped = NATIVE_FLOW_NAME_BY_BUTTON[button?.name];
        const nameAllowed = mapped != null ? SUPPORTED_NATIVE_FLOW_NAMES.has(mapped) : true;
...
const warnOnNativeFlowConstraints = (buttons, logger) => {
    ...
    const first = buttons[0]?.name;
    const limit = first === 'quick_reply' ? QUICK_REPLY_BUTTON_LIMIT : NATIVE_FLOW_BUTTON_LIMIT;
