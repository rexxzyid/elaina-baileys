...
import { downloadContentFromMessage, encryptedStream, generateThumbnail, getUrlFromDirectPath, getAudioDuration, getAudioWaveform, getImageProcessingLibrary, getRawMediaUploadData, getStream, toBuffer } from './messages-media.js';
import { shouldIncludeReportingToken } from './reporting-utils.js';
import { buildMusicMessage, normalizeStickers } from './status-stickers.js';

/**
 * The media upload answers with direct_path and often no url at all, so reading
 * url alone leaves the field empty and the music bubble never draws.
 */
const mediaUrlOf = (media, label) => {
    const url = media?.url || (media?.directPath ? getUrlFromDirectPath(media.directPath) : undefined);
    if (!url) {
        throw new Boom(`the ${label} upload returned neither a url nor a directPath`, { statusCode: 400 });
    }
    return url;
};
...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'raw')) {
        ...
        const { audio, artwork, ...music } = message.music;
        if (audio) {
            const { audioMessage } = await prepareWAMessageMedia({ audio, mimetype: 'audio/mp4' }, options);
            music.songUri = mediaUrlOf(audioMessage, 'audio');
        }
        if (artwork) {
            const { imageMessage } = await prepareWAMessageMedia({ image: artwork }, options);
            music.artworkUri = mediaUrlOf(imageMessage, 'artwork');
        }
        m.musicMessage = buildMusicMessage(music);
    }
    else {
