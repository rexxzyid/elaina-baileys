...
export const makeMessagesRecvSocket = (config) => {
    const { logger, retryRequestDelayMs, maxMsgRetryCount, ackRetryDelayMs, maxAckRetryCount, getMessage, shouldIgnoreJid, enableAutoSessionRecreation } = config;
    const sock = makeMessagesSocket(config);
    const { userDevicesCache, devicesMutex, ev, authState, ws, messageMutex, notificationMutex, receiptMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, sendPeerDataOperationMessage, messageRetryManager, registerSocketEndHandler, issuePrivacyTokens, fetchAccountReachoutTimelock, placeholderResendCache, outboundResendCache } = sock;
    const getLIDForPN = signalRepository.lidMapping.getLIDForPN.bind(signalRepository.lidMapping);
    /** this mutex ensures that each retryRequest will wait for the previous one to finish */
    const retryMutex = makeMutex();
    /**
     * How many retry requests are waiting on retryMutex. The lock is held for
     * retryRequestDelayMs per request, so the drain rate is fixed while the
     * arrival rate is not; without a bound a burst of undecryptable messages
     * queues indefinitely and every waiting task keeps its stanza alive.
     */
    let retryQueueDepth = 0;
    const maxRetryQueueSize = config.maxRetryQueueSize ?? DEFAULT_CONNECTION_CONFIG.maxRetryQueueSize;
    ...
    const handleMexNotification = async (node) => {
        ...
        if (updateNode) {
            ...
            switch (opName) {
                ...
                case 'NotificationNewsletterPaidPartnershipUpdate':
                case 'NotificationNewsletterMilestone':
                case 'NewsletterResponseStateUpdate':
                    await handleLegacyMexNewsletterNotification(node);
                    break;
                default:
                    logger.info({ opName }, 'unmodelled mex notification, emitting raw');
                    ev.emit('mex.notification', { operation: opName, data });
                    break;
            }
            return;
        }
    ...
    const handleLegacyMexNewsletterNotification = async (node) => {
        ...
        switch (operation) {
            ...
            case 'NotificationLinkedProfilesUpdates':
                ...
                break;
            default:
                logger.info({ operation, data }, 'unmodelled mex notification, emitting raw');
                ev.emit('mex.notification', { operation, updates, data });
                break;
        }
    };
            ...
            await messageMutex.mutex(async () => {
                ...
                if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== 'peer') {
                    ...
                    if (msg.messageStubParameters?.[0] === NO_MESSAGE_FOUND_ERROR_TEXT) {
                        ...
                        // retryMutex serializes every retry request across the whole
                        // socket, and each one holds the lock for retryRequestDelayMs.
                        // That caps retries at 1000/retryRequestDelayMs per second, so a
                        // burst of undecryptable messages queues up faster than it drains
                        // and every waiting task pins its stanza in memory. Refuse to
                        // queue past the bound: acking without a retry loses one message
                        // recovery, growing the queue without limit loses the process.
                        if (retryQueueDepth >= maxRetryQueueSize) {
                            logger.warn({ msgId: msg.key.id, retryQueueDepth, maxRetryQueueSize }, 'retry queue full, acking without retry');
                            acked = true;
                        ...
                        try {
                            await retryMutex.mutex(async () => {
                                try {
                                    if (!ws.isOpen) {
                                        logger.debug({ msgId: msg.key.id }, 'Connection closed, skipping retry');
                                        return;
                                    }
                                    const encNode = getBinaryNodeChild(node, 'enc');
    ...
    const ackRetryCounts = new NodeCache({ stdTTL: DEFAULT_CACHE_TTLS.MSG_RETRY, useClones: false });
    const resendAfterRetryableNack = async (key, attempt) => {
        // The socket's own copy comes first so a resend does not depend on the
        // caller having implemented getMessage. The store is the fallback for
        // anything older than the outbound cache keeps.
        const remembered = outboundResendCache?.get(key.id);
        const message = remembered?.message || (await getMessage(key).catch(() => undefined));
        if (!message) {
            logger.warn({ msgId: key.id }, 'cannot resend after retryable nack: message not in outbound cache and getMessage returned nothing');
            return false;
        }
        await delay(ackRetryDelayMs * attempt);
        await relayMessage(remembered?.jid || key.remoteJid, message, { messageId: key.id, useUserDevicesCache: false });
        return true;
    };
    const handleBadAck = async ({ attrs }) => {
        const key = { remoteJid: attrs.from, fromMe: true, id: attrs.id };
        if (!attrs.error) {
            ackRetryCounts.del(attrs.id);
        }
        else if (RETRYABLE_ACK_CODES.has(attrs.error)) {
            // The counter is deliberately left in place once the budget is spent.
            // Clearing it here would let the next nack for the same message start
            // over at attempt 1, which is an unbounded resend loop while the server
            // keeps rejecting. The cache TTL is what eventually frees the entry.
            const attempt = (ackRetryCounts.get(attrs.id) || 0) + 1;
            ackRetryCounts.set(attrs.id, attempt);
            if (attempt <= maxAckRetryCount) {
                logger.info({ msgId: attrs.id, error: attrs.error, attempt, maxAckRetryCount }, 'retryable nack, resending message');
                const resent = await resendAfterRetryableNack(key, attempt)
                    .catch(error => {
                    logger.warn({ err: error, msgId: attrs.id }, 'resend after retryable nack failed');
                    return false;
                });
                if (resent) {
                    return;
                ...
                logger.warn({ msgId: attrs.id, error: attrs.error, attempt }, 'retryable nack gave up after max attempts');
            }
        }
        ...
        if (attrs.error) {
            ...
            else if (attrs.error === SERVER_ERROR_CODES.NewChatMessagesCapped) {
                // 475 = the new-chat quota is spent. Existing conversations keep working,
                // so this is a throttle on reaching new people, not a broken message.
                // No retry — a resend is another new chat and spends nothing but goodwill.
                logger.warn({ msgId: attrs.id, from: attrs.from }, 'error 475: new chat message quota reached');
                ev.emit('message-capping.update', { status: 'CAPPED', jid: attrs.from, messageId: attrs.id });
            }
