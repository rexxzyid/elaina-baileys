...
const processNode = (type, node, identifier, exec) => {
    ...
    ws.on('CB:notification', async (node) => {
        try {
            ...
            // If server tries to initiate passkey flow, ack and skip normal processing
            if (notifType === 'passkey_prologue_request' || notifType === 'crsc_continuation') {
                try {
                    // Log and ack the notification to avoid server retries
                    if (typeof logger !== 'undefined' && logger.info) {
                        logger.info({ notifType }, 'Received passkey notification - acknowledging and skipping automatic processing');
                    }
                    // send a basic ack stanza for notification so server knows we handled it
                    await sendNode({
                        tag: 'ack',
                        attrs: {
                    ...
                    if (typeof logger !== 'undefined' && logger.debug) {
                        logger.debug({ err }, 'Failed to ack passkey notification');
                    }
                }

                // Emit an explicit event for the embedding app to optionally handle passkey flows
                try {
                    if (typeof ev !== 'undefined' && typeof ev.emit === 'function') {
                        const eventName = notifType === 'passkey_prologue_request' ? 'passkey.request' : 'passkey.continuation';
                        ev.emit(eventName, { node });
                    }
                } catch (emitErr) {
                    if (typeof logger !== 'undefined' && logger.debug) {
                        logger.debug({ emitErr }, 'Error emitting passkey event');
                    }
                }
                return;
            }
        } catch (err) {
            // Non-fatal: fallthrough to normal processing
            if (typeof logger !== 'undefined' && logger.debug) {
