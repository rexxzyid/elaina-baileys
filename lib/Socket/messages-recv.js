...
export const makeMessagesRecvSocket = (config) => {
    ...
    const resendAfterRetryableNack = async (key, attempt) => {
        const message = await getMessage(key).catch(() => undefined);
        if (!message) {
            logger.warn({ msgId: key.id, error: 'no stored message' }, 'cannot resend after retryable nack');
            return false;
        }
        await delay(ackRetryDelayMs * attempt);
        await relayMessage(key.remoteJid, message, { messageId: key.id, useUserDevicesCache: false });
        return true;
    };
