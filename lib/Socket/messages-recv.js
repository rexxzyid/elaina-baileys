...
const passkey_1 = require("./passkey-handler");
const passkeyHandler = new passkey_1.PasskeyHandler();                
...
const makeMessagesRecvSocket = (config) => {
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'link_code_companion_reg':
                ...
                if (!linkCodeCompanionReg || !(0, WABinary_1.getBinaryNodeChildBuffer)(linkCodeCompanionReg, 'link_code_pairing_ref')) {
                    logger.debug({ node }, 'skipping link_code_companion_reg notification without pairing data');
                    break;
                }
    ...
    if (typeof exec !== 'function') {
        try {
            logger && logger.warn && logger.warn({ identifier, nodeTag: node?.tag }, 'processNodeWithBuffer called without a handler; skipping');
        }
        catch (_) { }
        return;
    }
    ...
    function execTask() {
        try {
            return Promise.resolve(exec(node, false)).catch(err => onUnexpectedError(err, identifier));
        }
        catch (err) {
...
const processNode = (type, node, identifier, exec) => {
    if (typeof identifier === 'function' && typeof exec === 'undefined') {
        exec = identifier;
        identifier = `processing ${type}`;
    }
    const isOffline = !!(node && node.attrs && node.attrs.offline);
    if (isOffline) {
    ...
    if (typeof exec !== 'function') {
        try {
            logger && logger.warn && logger.warn({ type, identifier }, 'no handler provided for node; skipping processing');
        }
        catch (_) { }
        return;
    }
...
// Intercept notification frames and specially handle passkey notifications
    ws.on('CB:notification', async (node) => {
    try {
        const attrs = node.attrs || {};
        const notifType = attrs.type;
        if (!notifType) {
            return processNode('notification', node, handleNotification);
        }

        if (notifType === 'passkey_prologue_request') {
            try {
                logger.info({ notifType }, 'Received passkey prologue request - auto-responding');
                
                await sendNode({
                    tag: 'ack',
                    attrs: {
                        id: attrs.id || generateMessageTag(),
                ...
                const optChild = getChild(node, 'passkey_request_options');
                let options = null;
                if (optChild && optChild.content) {
                    try {
                        const optionsJSON = optChild.content.toString('utf-8');
                        options = JSON.parse(optionsJSON);
                        logger.debug({ options }, 'Parsed passkey options');
                    } catch (parseErr) {
                        logger.debug({ parseErr }, 'Failed to parse passkey options');
                    }
                }
                ...
                const challenge = options && options.challenge 
                    ? Buffer.from(options.challenge, 'base64').toString('hex')
                    : crypto.randomBytes(32).toString('hex');
                
                const clientDataJSON = JSON.stringify({
                    type: 'webauthn.get',
                    challenge: Buffer.from(challenge, 'hex').toString('base64url').replace(/=/g, ''),
                ...
                const webauthnResponse = JSON.stringify({
                    id: credentialID.toString('base64url').replace(/=/g, ''),
                    rawId: credentialID.toString('base64url').replace(/=/g, ''),
                    type: 'public-key',
                    response: {
                        clientDataJSON: Buffer.from(clientDataJSON).toString('base64url').replace(/=/g, ''),
                        authenticatorData: authenticatorData.toString('base64url').replace(/=/g, ''),
                        signature: signature.toString('base64url').replace(/=/g, ''),
                        userHandle: null
                    }
                });
                ...
                await query({
                    ...
                    content: [{
                        ...
                        content: [
                            ...
                            { tag: 'webauthn_assertion', attrs: {}, content: Buffer.from(webauthnResponse) }
                        ]
                    }]
                });

                logger.info('Passkey prologue response sent successfully');
                ev.emit('passkey.request', { node, options });
            } catch (err) {
                logger.error({ err }, 'Error handling passkey prologue');
            }
            return;
        }

        if (notifType === 'crsc_continuation') {
            try {
                logger.info({ notifType }, 'Received passkey continuation - auto-responding');
                ...
                const identChild = getChild(node, 'primary_ephemeral_identity');
                let code = null;
                
                if (identChild && identChild.content) {
                    try {
                        const identBuffer = Buffer.isBuffer(identChild.content) 
                            ? identChild.content 
                            : Buffer.from(identChild.content);
                        
                        const primaryEphemeralIdentity = {
                            publicKey: identBuffer.slice(0, 32),
                            nonce: identBuffer.slice(32, 64)
                        };

                        code = passkeyHandler.handleContinuation(primaryEphemeralIdentity);
                        logger.info({ code }, 'Computed passkey confirmation code');
                    } catch (parseErr) {
                        logger.error({ parseErr }, 'Failed to parse passkey continuation');
                    }
                }

                const companionNonce = passkeyHandler.getCompanionNonce();
                ...
                await query({
                    ...
                    content: [{
                        tag: 'companion_nonce',
                        attrs: {},
                        content: companionNonce
                    }]
                });

                logger.info('Passkey companion nonce sent successfully');
                ev.emit('passkey.continuation', { node, code });
            } catch (err) {
                logger.error({ err }, 'Error handling passkey continuation');
            }
            return;
        }
    } catch (err) {
        if (typeof logger !== 'undefined' && logger.debug) {
            logger.debug({ err }, 'error checking notification type');
        }
    }
    processNode('notification', node, handleNotification);
});
