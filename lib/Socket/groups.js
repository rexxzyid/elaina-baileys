...
export const extractGroupMetadata = (result) => {
    ...
    const metadata = {
        ...
        subjectOwner: group.attrs.s_o ? jidNormalizedUser(group.attrs.s_o) : undefined,
        ...
        participants: getBinaryNodeChildren(group, 'participant').map(({ attrs }) => {
            const jid = attrs.jid;
            const lid = isLidUser(jid) ? jid : isLidUser(attrs.lid) ? attrs.lid : undefined;
            const phoneNumber = isPnUser(jid) ? jid : isPnUser(attrs.phone_number) ? attrs.phone_number : undefined;
            return {
                id: jid,
                phoneNumber,
                lid,
