...
const NEWSLETTER_SERVER_ID_MIN = 99;
const NEWSLETTER_SERVER_ID_MAX = 2147476647;
...
export const toNewsletterServerIds = (serverIds) => {
    ...
    return list.map(id => {
        ...
        if (!Number.isInteger(parsed) || parsed < NEWSLETTER_SERVER_ID_MIN || parsed > NEWSLETTER_SERVER_ID_MAX) {
            throw new TypeError(`${JSON.stringify(id)} is not a newsletter server id; the server accepts ${NEWSLETTER_SERVER_ID_MIN}..${NEWSLETTER_SERVER_ID_MAX}, and you get one from newsletterFetchMessages or a sent status`);
        }
