...
export const makeMessagesSocket = (config) => {
    ...
    const newMessageId = (userId) => {
        const custom = config.messageIdGenerator;
        if (typeof custom !== 'function') {
            return generateMessageIDV2(userId);
        }
        const id = custom(userId);
        if (typeof id !== 'string' || id.length === 0) {
            throw new Boom('messageIdGenerator must return a non-empty string', { statusCode: 400 });
        }
        return id;
    };
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                if (config.inlineSenderKeyDistribution && isGroup && !patched.senderKeyDistributionMessage) {
                    try {
                        ...
                        logger.warn({ err, jid: destinationJid }, 'failed to inline SKDM, sending without it');
                    }
                }
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (Array.isArray(jid)) {
                ...
                const fullMsg = await generateWAMessage('status@broadcast', content, {
                    ...
                    ...options,
                    messageId: newMessageId(userJid)
                });
                for (const id of jid) {
                    if (isJidGroup(id)) {
                        try {
                ...
                if (pollOptionImages.length) {
                    for (const optionImage of pollOptionImages) {
                        ...
                        }, { userJid, messageId: newMessageId(userJid) });
