...
export const makeMessagesSocket = (config) => {
    ...
    /**
     * Keeps the content of recently relayed messages so a retryable nack can be
     * resent without asking the caller's store for it. A nack lands seconds after
     * the send, so the entries are short lived, and the key cap bounds the memory
     * a high volume bot can hold here.
     */
    ...
    const rememberOutboundMessage = (messageId, jid, message) => {
        ...
        try {
            ...
            // The cache is full. Losing an entry only costs us the store-free
            // resend path, so this must never interrupt the send itself.
        }
    };
                ...
                if (pollOptionImages.length) {
                    for (const optionImage of pollOptionImages) {
                        const optionMsg = generateWAMessageFromContent(jid, {
                            ...
                            messageContextInfo: {
                                // generateWAMessageFromContent takes content that is already
                                // built, so it skips the generateWAMessageContent step that
                                // would normally add this. Without a secret the server keeps
                                // the message but drops its link to the poll, and the option
