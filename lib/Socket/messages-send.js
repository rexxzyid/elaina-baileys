...
import { BIZ_BOT_SUPPORT_PAYLOAD, DEFAULT_CACHE_TTLS, MAX_OUTBOUND_RESEND_ENTRIES, MEMBER_LABEL_MAX_LENGTH, WA_DEFAULT_EPHEMERAL } from '../Defaults/index.js';
import { aggregateMessageKeysNotFromMe, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, DEF_MEDIA_HOST, delay, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, generateWAMessageFromContent, getPollOptionHash, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, hasValidAlbumMedia, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, prepareWAMessageMedia, shouldIncludeBizBinaryNode, unixTimestampSeconds } from '../Utils/index.js';
import { AssociationType } from '../Types/index.js';
import { makeStatusMentionMessage } from '../Utils/modern-messages.js';
import { commitSenderKeyDelivery, pickSenderKeyRecipients, senderKeyResetSummary } from '../Utils/sender-key-memory.js';
import { describeUnusableGroupMetadata, isUsableGroupMetadata } from '../Utils/group-metadata-cache.js';
...
export const buildMemberLabelMessage = (jid, memberLabel) => {
    if (!isJidGroup(jid)) {
        throw new Boom('a member label belongs to a group, so jid must be a group jid', { statusCode: 400, data: { jid } });
    }
    const label = memberLabel === undefined || memberLabel === null ? '' : String(memberLabel).slice(0, MEMBER_LABEL_MAX_LENGTH);
    return {
        label,
        content: {
            protocolMessage: {
        ...
        node: {
            tag: 'meta',
            attrs: {
                tag_reason: label ? 'user_update' : 'user_delete',
                appdata: 'member_tag'
            },
            content: undefined
        }
...
export const makeMessagesSocket = (config) => {
    ...
    const userDevicesCache = config.userDevicesCache ||
        new TTLCache({ stdTTL: DEFAULT_CACHE_TTLS.USER_DEVICES });

    const devicesMutex = makeMutex();

    const outboundResendCache = config.outboundResendCache ||
        new TTLCache({ stdTTL: DEFAULT_CACHE_TTLS.OUTBOUND_RESEND, maxKeys: MAX_OUTBOUND_RESEND_ENTRIES });
    const rememberOutboundMessage = (messageId, jid, message) => {
        if (!messageId || !message) {
            return;
        }
        try {
            outboundResendCache.set(messageId, { jid, message });
        }
        catch {
    ...
    const updateMemberLabel = async (jid, memberLabel, options = {}) => {
        const { label, content, node } = buildMemberLabelMessage(jid, memberLabel);
        const messageId = await relayMessage(jid, content, {
            ...options,
            additionalNodes: [node, ...(options.additionalNodes ?? [])]
        });
        return { jid, label, messageId };
    };

    const removeMemberLabel = (jid, options = {}) => updateMemberLabel(jid, '', options);
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
        ...
        if (!isRetryResend) {
            rememberOutboundMessage(msgId, jid, message);
        }
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                const [groupData, senderKeyMap] = await Promise.all([
                    (async () => {
                        ...
                        if (isUsableGroupMetadata(groupData)) {
                            logger.trace({ jid, participants: groupData.participants.length, addressingMode: groupData.addressingMode }, 'using cached group metadata');
                        }
                        else if (!isStatus) {
                            if (groupData) {
                                logger.debug({ jid, reason: describeUnusableGroupMetadata(groupData) }, 'cached group metadata is not usable, fetching it so the addressing mode is not guessed');
                            }
                ...
                let toEncode = patched;
                if (config.inlineSenderKeyDistribution !== false && isGroup && !patched.senderKeyDistributionMessage) {
                    try {
                        if (await signalRepository.hasSenderKey({ group: destinationJid, meId: groupSenderIdentity })) {
                            const inlineSkdm = await signalRepository.getSenderKeyDistributionMessage({
                            ...
                            toEncode = {
                                ...
                                senderKeyDistributionMessage: {
                                    ...
                                    axolotlSenderKeyDistributionMessage: inlineSkdm
                                }
                            };
                        }
                    }
                    catch (err) {
                        logger.warn({ err, jid: destinationJid }, 'failed to inline the sender key distribution message');
                    }
                }
                const bytes = encodeWAMessage(toEncode);
                reportingMessage = toEncode;
                const { ciphertext, senderKeyDistributionMessage } = await signalRepository.encryptGroupMessage({
                    group: destinationJid,
                    data: bytes,
                    meId: groupSenderIdentity
                });
                const senderKeyRecipients = pickSenderKeyRecipients(devices, senderKeyMap, {
                    force: !!participant,
                    skip: device => isHostedLidUser(device.jid) || isHostedPnUser(device.jid) || device.device === 99
                });
                if (senderKeyRecipients.length) {
                    ...
                    const { skipped } = commitSenderKeyDelivery(senderKeyMap, senderKeyRecipients, result.nodes);
                    if (skipped.length) {
                        logger.warn({ jid, skipped }, 'sender key did not reach every device, leaving them unmarked so the next send retries');
                    }
                }
                binaryNodeContent.push({
                    tag: 'enc',
            ...
            if (!isNewsletter &&
                ...
                try {
                    const encoded = proto.Message.encode(reportingMessage).finish();
                    const reportingKey = {
                        id: msgId,
                        fromMe: true,
    ...
    return {
        ...
        getUSyncDevices,
        messageRetryManager,
        resetGroupSenderKey: async (jid) => {
            if (!isJidGroup(jid)) {
                throw new Boom('resetGroupSenderKey needs a group jid', { statusCode: 400 });
            }
            const stored = await authState.keys.get('sender-key-memory', [jid]);
            const summary = senderKeyResetSummary(stored, jid);
            await authState.keys.set({ 'sender-key-memory': { [jid]: null } });
            logger.info(summary, 'cleared sender key memory, the next send redistributes the key');
            return summary;
        },
        updateMemberLabel,
        removeMemberLabel,
            ...
            if (Array.isArray(jid)) {
                ...
                for (const id of jid) {
                    ...
                    const mentionMsg = generateWAMessageFromContent(id, {
                        ...
                        ...makeStatusMentionMessage({ key: fullMsg.key, group: isGroup })
                    }, {
                        userJid
                    });
                ...
                const isQuizMsg = isPollMsg && !!content.poll.pollType;
                const isPhotoPollMsg = isPollMsg && Array.isArray(content.poll.values) && content.poll.values.some(value => value && typeof value === 'object' && value.image);
                const pollOptionImages = [];
                if (isPhotoPollMsg) {
                    ...
                    for (const value of content.poll.values) {
                        ...
                        const prepared = await prepareWAMessageMedia({ image: value.image }, {
                            logger,
                            upload: waUploadToServer,
                            mediaCache: config.mediaCache,
                            options: config.options,
                        ...
                        pollOptionImages.push({ content: prepared });
                    }
                    content = { ...content, poll: { ...content.poll, values: preparedValues } };
                }
                ...
                if (isDeleteMsg || isKeepMsg) {
                    ...
                    additionalNodes.push({
                        ...
                        attrs: {
                            ...
                            contenttype: isPhotoPollMsg ? 'image' : isPollMsg && isNewsletter ? 'text' : undefined
                        },
                        content: undefined
                    });
                }
                else if (isPollResultMsg) {
                    additionalNodes.push({
                        tag: 'meta',
                        attrs: {
                ...
                if (pollOptionImages.length) {
                    for (const optionImage of pollOptionImages) {
                        const optionMsg = generateWAMessageFromContent(jid, {
                            pollCreationOptionImageMessage: { message: optionImage.content },
                            messageContextInfo: {
                                messageAssociation: {
                                    parentMessageKey: fullMsg.key,
                                    associationType: AssociationType.MEDIA_POLL
                                }
                            }
                        }, { userJid, messageId: generateMessageIDV2(userJid) });
