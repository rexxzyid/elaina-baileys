...
const buildButton = (b = {}) => {
    ...
    switch (type) {
        case 'reply':
        case 'quick_reply':
            return { name: 'quick_reply', buttonParamsJson: JSON.stringify({ display_text: b.text != null ? b.text : (b.display_text || ''), id: b.id != null ? b.id : (b.text || '') }) };
        case 'url':
        case 'link':
        case 'cta_url':
            return { name: 'cta_url', buttonParamsJson: JSON.stringify({ display_text: b.text || '', url: b.url || '', merchant_url: b.merchant_url || b.url || '' }) };
        case 'copy':
        case 'cta_copy':
            return { name: 'cta_copy', buttonParamsJson: JSON.stringify({ display_text: b.text || 'Copy', id: b.id || b.text || 'copy', copy_code: b.copy || b.code || b.copy_code || '' }) };
        case 'call':
        case 'cta_call':
            return { name: 'cta_call', buttonParamsJson: JSON.stringify({ display_text: b.text || 'Call', phone_number: b.phone || b.number || b.phone_number || '' }) };
        case 'list':
        case 'select':
        case 'single_select':
            return { name: 'single_select', buttonParamsJson: JSON.stringify({ title: b.text || b.title || 'Menu', sections: b.sections || [] }) };
        case 'location':
        case 'send_location':
            return { name: 'send_location', buttonParamsJson: '' };
        case 'reminder':
        case 'cta_reminder':
            return { name: 'cta_reminder', buttonParamsJson: JSON.stringify({ display_text: b.text || 'Reminder', id: b.id || b.text || 'reminder' }) };
        case 'address':
        case 'address_message':
            return { name: 'address_message', buttonParamsJson: JSON.stringify({ display_text: b.text || 'Address', id: b.id || b.text || 'address' }) };
        default:
            return { name: b.name || type, buttonParamsJson: typeof b.params === 'string' ? b.params : JSON.stringify(b.params || {}) };
    }
};

const AI_HL = { default: 0, keyword: 1, method: 2, string: 3, number: 4, comment: 5 };
const AI_PROVIDER = { unknown: 0, bing: 1, google: 2, support: 3, other: 4 };
const AI_ALIGN = { leading: 0, left: 0, trailing: 1, right: 1, center: 2, centered: 2 };
const AI_DYN = { unknown: 0, image: 1, gif: 2 };
/**
 * buildRichBlock — ubah blok ramah { type, ... } jadi AIRichResponseSubMessage.
 * type: text | code | table | images/grid | image/inline | reels/carousel | latex | map | dynamic/gif
 */
const buildRichBlock = (b = {}) => {
    const type = String(b.type || 'text').toLowerCase();
    switch (type) {
        case 'text':
            return { messageType: 2, messageText: b.text != null ? b.text : (b.messageText || '') };
        case 'code': {
            const blocks = b.code != null ? b.code : b.blocks;
            const codeBlocks = typeof blocks === 'string'
                ? [{ codeContent: blocks, highlightType: 0 }]
                : (blocks || []).map(x => ({
            ...
            return { messageType: 5, codeMetadata: { codeLanguage: b.language || b.lang || '', codeBlocks } };
        }
        case 'table': {
            const rows = (b.rows || []).map((r, i) => Array.isArray(r)
                ? ({ items: r, isHeading: !!(b.header && i === 0) })
                : ({ items: r.items || [], isHeading: !!r.isHeading }));
            return { messageType: 4, tableMetadata: { title: b.title || '', rows } };
        }
        case 'images':
        case 'grid': {
            const imageUrls = (b.urls || b.images || []).map(u => typeof u === 'string'
                ? { imagePreviewUrl: u, imageHighResUrl: u, sourceUrl: b.sourceUrl || '' } : u);
            return { messageType: 1, gridImageMetadata: { gridImageUrl: { imagePreviewUrl: imageUrls[0] && imageUrls[0].imagePreviewUrl }, imageUrls } };
        }
        case 'image':
        case 'inline':
        case 'inlineimage': {
            const url = b.url || (b.imageUrl && b.imageUrl.imagePreviewUrl) || '';
            return { messageType: 3, imageMetadata: {
                imageUrl: { imagePreviewUrl: url, imageHighResUrl: b.highRes || url, sourceUrl: b.sourceUrl || '' },
                imageText: b.text || '',
        ...
        case 'content':
        case 'carousel': {
            const itemsMetadata = (b.items || []).map(it => ({ reelItem: {
                title: it.title || '',
                profileIconUrl: it.icon || it.profileIconUrl || '',
                thumbnailUrl: it.thumbnail || it.thumbnailUrl || '',
                videoUrl: it.video || it.videoUrl || '',
            } }));
            return { messageType: 9, contentItemsMetadata: { contentType: b.carousel === false ? 0 : 1, itemsMetadata } };
        }
        case 'latex': {
            const expressions = (b.expressions || []).map(e => ({
                latexExpression: e.latex || e.latexExpression || '',
                url: e.url || '', width: e.width, height: e.height, fontHeight: e.fontHeight,
            }));
            return { messageType: 8, latexMetadata: { text: b.text || '', expressions } };
        }
        case 'map': {
            ...
            return { messageType: 7, mapMetadata: {
                ...
                annotations, showInfoList: !!b.showInfoList,
            } };
        }
        case 'dynamic':
        case 'gif': {
            return { messageType: 6, dynamicMetadata: {
                type: b.kind != null ? (AI_DYN[String(b.kind).toLowerCase()] ?? 0) : (type === 'gif' ? 2 : 1),
                version: b.version || 1, url: b.url || '', loopCount: b.loopCount || 0,
            } };
        }
        default:
            
            return Object.assign({ messageType: b.messageType || 0 }, b);
    }
};

const AI_UNI_HL_NAME = { default: 'DEFAULT', keyword: 'KEYWORD', method: 'METHOD', string: 'STR', number: 'NUMBER', comment: 'COMMENT' };
const AI_UNI_HL_NUM = ['DEFAULT', 'KEYWORD', 'METHOD', 'STR', 'NUMBER', 'COMMENT'];
const _uniSingle = (primitive) => ({ view_model: { primitive, __typename: 'GenAISingleLayoutViewModel' } });
/** ubah satu block -> satu/lebih "section" unified (array, karena grid = banyak section) */
const buildUnifiedSection = (b = {}) => {
    const type = String(b.type || 'text').toLowerCase();
    switch (type) {
        case 'text':
            return [_uniSingle({ text: b.text != null ? b.text : (b.messageText || ''), inline_entities: b.inlineEntities || b.inline_entities || [], __typename: 'GenAIMarkdownTextUXPrimitive' })];
        case 'code': {
            const blocks = b.code != null ? b.code : b.blocks;
            const code_blocks = typeof blocks === 'string'
                ? [{ content: blocks, type: 'DEFAULT' }]
                : (blocks || []).map(x => ({
            ...
            return [_uniSingle({ language: b.language || b.lang || '', code_blocks, __typename: 'GenAICodeUXPrimitive' })];
        }
        case 'table': {
            const rows = (b.rows || []).map((r, i) => Array.isArray(r)
                ? ({ is_header: !!(b.header && i === 0), cells: r })
                : ({ is_header: !!r.isHeading, cells: r.items || [] }));
            return [_uniSingle({ rows, __typename: 'GenATableUXPrimitive' })]; 
        }
        case 'images':
        case 'grid': {
            ...
            return (b.urls || b.images || []).map(u => {
                ...
                return _uniSingle({ media: { url, mime_type: b.mime || 'image/jpeg' }, imagine_type: 3, status: { status: 'READY' }, __typename: 'GenAIImaginePrimitive' });
            });
        }
        case 'image':
        case 'inline':
        ...
        case 'searchresult': {
            const sources = (b.items || b.sources || []).map(s => ({
                ...
                source_url: s.url || '',
                favicon: { url: s.favicon || s.url || '', mime_type: 'image/jpeg', width: 16, height: 16 },
            }));
            return [_uniSingle({ sources, __typename: 'GenAISearchResultPrimitive' })];
        }
        default:
...
const buildUnified = (blocks, opts = {}) => {
    ...
    if (!rid) { try { rid = require('crypto').randomUUID(); } catch (_) { rid = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, c => { const r = Math.random() * 16 | 0; return (c === 'x' ? r : (r & 0x3 | 0x8)).toString(16); }); } }
    return Buffer.from(JSON.stringify({ response_id: rid, sections }), 'utf8').toString('base64');
};
/** buildRichContent — rakit objek final AI rich response utk relayMessage */
const buildRichContent = (content = {}) => {
    const submessages = Array.isArray(content.blocks) ? content.blocks.map(buildRichBlock) : [];
    const richResponseMessage = { messageType: 1, submessages };
    if (content.unified || content.unifiedResponse)
        richResponseMessage.unifiedResponse = { data: content.unified || content.unifiedResponse };
    else if (content.autoUnified !== false)
        richResponseMessage.unifiedResponse = { data: buildUnified(content.blocks, content) };
    richResponseMessage.contextInfo = { forwardingScore: 1, isForwarded: true };
    const _botJid = content.botJid === undefined ? '0@bot' : content.botJid;
    if (_botJid)
        richResponseMessage.contextInfo.forwardedAiBotMessageInfo = { botJid: _botJid };
    if (content.contextInfo) Object.assign(richResponseMessage.contextInfo, content.contextInfo);
    const botMetadata = { messageDisclaimerText: content.disclaimer || 'Elaina Multidevice' };
    if (Array.isArray(content.sources) && content.sources.length) {
        botMetadata.richResponseSourcesMetadata = { sources: content.sources.map((s, i) => ({
...
const TUTOR_BUTTON = [
    '📘 TUTORIAL sock.sendButton',
    '',
    'Kirim tombol interaktif (native-flow). Format:',
    "  await sock.sendButton(jid, { text, footer, title, buttons:[...] }, { quoted })",
    '',
    'TIPE TOMBOL (isi field "type"):',
    ...
    "  select  -> { type:'select', text:'Menu',  sections:[{ title:'A', rows:[{ title:'Ping', description:'cek', id:'.ping' }] }] }",
    "  location-> { type:'location', text:'Lokasi' }",
    "  reminder-> { type:'reminder', text:'Ingatkan', id:'rmd1' }",
    "  address -> { type:'address',  text:'Alamat',   id:'addr1' }",
    '',
    'CONTOH LENGKAP:',
    ...
    "  }, { quoted: m })",
    '',
    'MEDIA + TOMBOL: ganti text -> image/video/document + caption:',
    "  await sock.sendButton(jid, { image:{url:'https://x.jpg'}, caption:'Halo', buttons:[...] })",
    '',
    'TIPE BARU (belum dipetakan): { name:'+"'nama_native_flow', params:{...} }",
].join('\n');
const TUTOR_AIRICH = [
    '📗 TUTORIAL sock.sendRichResponse (AI Rich Message)',
    '',
    'Format:',
    "  await sock.sendRichResponse(jid, { disclaimer, sources:[...], blocks:[...] })",
    '  (botJid otomatis 0@bot; set botJid:false untuk menghilangkan)',
    '',
    'TIPE BLOCK (isi field "type"):',
    "  text  -> { type:'text',  text:'# Markdown boleh' }",
    "  code  -> { type:'code',  language:'javascript', code:'const a = 1' }",
    "           (berwarna: code:[{content:'const',type:'keyword'},{content:' a',type:'default'}])",
    "           type highlight: keyword|method|string|number|comment|default",
    "  table -> { type:'table', rows:[['Nama','Role'],['Rexx','Dev']], header:true, title:'' }",
    "  images-> { type:'images', urls:['https://a.jpg','https://b.jpg'] }",
    "  image -> { type:'image', url:'https://c.jpg', text:'caption', align:'center', link:'https://x/' }",
    "           align: leading|center|trailing",
    "  reels -> { type:'reels', items:[{ title:'Demo', thumbnail:'https://t.jpg', video:'https://v/', icon:'https://i.jpg' }] }",
    "  latex -> { type:'latex', text:'rumus', expressions:[{ latex:'x^2', url:'https://l/', width:10, height:10 }] }",
    "  map   -> { type:'map', lat:-6.2, lng:106.8, annotations:[{ lat:-6.2, lng:106.8, title:'Jakarta' }], showInfoList:true }",
    "  gif   -> { type:'gif', url:'https://g.gif', loopCount:3 }   (atau type:'dynamic', kind:'image'|'gif')",
    '',
    'SOURCES / CITATION (opsional):',
    "  sources:[{ title:'RexxHayanasi', url:'https://x/', provider:'OTHER' }]",
    "  provider: UNKNOWN|BING|GOOGLE|SUPPORT|OTHER",
    '',
    'UNIFIED (base64) — OTOMATIS:',
    '  unifiedResponse.data dibuat OTOMATIS dari blocks (default ON, tak perlu tempel base64).',
    '  matikan : autoUnified:false   (cukup submessages)',
    "  manual  : unified:'<base64>'  (pakai base64 buatanmu, menang atas auto)",
    '  Catatan : latex/map/gif belum ada padanan unified (hanya di submessages).',
    '',
    'KARTU SUMBER di unified (beda dari citation sources di atas):',
    "  { type:'sources', items:[{ name:'GitHub', url:'https://..', subtitle:'AI', favicon:'https://..' }] }",
    '',
    'CONTOH LENGKAP:',
    "  await sock.sendRichResponse(m.chat, {",
    "    disclaimer: 'Elaina Multidevice',",
    "    sources: [{ title:'RexxHayanasi', url:'https://RexxHayanasi.my.id/', provider:'OTHER' }],",
    "    blocks: [",
    "      { type:'text',  text:'# Halo Dunia' },",
    "      { type:'code',  language:'javascript', code:\"class Rexx { static hi(){ return 'Hi' } }\" },",
    "      { type:'table', rows:[['Nama','Role'],['Rexx','Dev']], header:true },",
    "      { type:'images', urls:['https://a.jpg','https://b.jpg'] },",
    "    ]",
    "  })",
].join('\n');
const makeMessagesSocket = (config) => {
    ...
    __socket.sendButton = (jid, content = {}, options = {}) => {
        ...
        const msg = { interactiveButtons: buttons };
        if (content.image) {
        ...
        if (content.footer)
            msg.footer = content.footer;
        if (content.title)
            msg.title = content.title;
        if (content.subtitle)
            msg.subtitle = content.subtitle;
        if (content.mentions)
            msg.mentions = content.mentions;
        if (content.contextInfo)
            msg.contextInfo = content.contextInfo;
    ...
    __socket.tutorbutton = (jid, options = {}) => {
        if (jid) return __socket.sendMessage(jid, { text: TUTOR_BUTTON }, options);
        return TUTOR_BUTTON;
    };
    __socket.tutorairich = (jid, options = {}) => {
        if (jid) return __socket.sendMessage(jid, { text: TUTOR_AIRICH }, options);
        return TUTOR_AIRICH;
    };
