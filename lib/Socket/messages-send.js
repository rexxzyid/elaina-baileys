...
const buildButton = (b = {}) => {
    ...
    switch (type) {
        ...
        default:
            // passthrough tipe baru/lanjutan: { name: 'nama_native_flow', params: {...} }
...
const extractIE = (text, { extract = true, hyperlink = true, citation = true, latex = true } = {}) => {
    if (!extract || typeof text !== 'string') return { text: text || '', inline_entities: [] };
    ...
    let inline_entities = [], result = '', last = 0, citation_index = 1, hyperlink_index = 0, latex_index = 0, stack = [];
...
const buildRichBlock = (b = {}) => {
    ...
    switch (type) {
        case 'text':
            return { messageType: 2, messageText: extractIE(b.text != null ? b.text : (b.messageText || '')).text };
        case 'code': {
        ...
        default:
            // passthrough: { type/messageType, ...rawFields }
...
const buildUnifiedSection = (b = {}) => {
    ...
    switch (type) {
        case 'text': {
            ...
            return [_uniSingle({ text: ie.text, ...(ie.inline_entities.length ? { inline_entities: ie.inline_entities } : {}), __typename: 'GenAIMarkdownTextUXPrimitive' })];
        }
        case 'code': {
            const blocks = b.code != null ? b.code : b.blocks;
        ...
        case 'table': {
            ...
            return [_uniSingle({ rows, __typename: 'GenATableUXPrimitive' })]; // NB: GenATable (memang tanpa "I")
        }
        case 'images':
        case 'grid': {
            // tiap gambar -> section GenAIImaginePrimitive tersendiri
...
const buildUnified = (blocks, opts = {}) => {
    ...
    if (Array.isArray(opts.sources) && opts.sources.length)
        sections.push(...buildUnifiedSection({ type: 'sources', items: opts.sources }));
    if (opts.footer)
        sections.push(_uniSingle({ text: opts.footer, __typename: 'GenAIMetadataTextPrimitive' }));
...
const buildRichContent = (content = {}) => {
    ...
    richResponseMessage.contextInfo = { forwardingScore: 1, isForwarded: true, forwardOrigin: 4 };
...
// === TUTORIAL STRINGS (dikembalikan oleh sock.tutorButton / sock.tutorAiRich) ===
const TUTOR_BUTTON = `╭───────────────────────────────╮
│  📘  sock.sendButton          │
╰───────────────────────────────╯
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                if (isLidGroup && groupData && Array.isArray(groupData.participants)) {
                    ...
                    // pastikan diri sendiri ikut terpetakan (telepon -> LID kita)
