...
const protocol_debug_1 = require("../Utils/protocol-debug");
...
const makeSocket = (config) => {
    ...
    const protocolDebug = new protocol_debug_1.ProtocolDebug(config);
    ...
    // Attach passkey helpers so callers can send passkey responses/confirmations
    try {
        passkey_1.attachPasskeyHandlers({
            sock,
            ev,
            logger,
            sendNode,
        ...
        // don't fail socket creation if passkey helper attachment errors
        logger.debug({ err }, 'failed to attach passkey handlers');
    }
