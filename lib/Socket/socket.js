...
const makeSocket = (config) => {
    ...
    // Fix: Issue - "Keep the app open on both devices / Continue on WhatsApp Web"
    ws.on('CB:ib,,passkey_challenge', async (node) => {
        logger.info({ node }, 'passkey_challenge recv — declining (not a browser client)');
        try {
            const challengeNode = (0, WABinary_1.getBinaryNodeChild)(node, 'passkey_challenge');
            const challengeId = challengeNode?.attrs?.id || node.attrs?.id || '';
            await sendNode({
                tag: 'ib',
                attrs: {},
                content: [{
                    tag: 'passkey_challenge',
                    attrs: {
                        result: 'decline',
            ...
            logger.info('passkey_challenge decline sent — waiting for CB:success');
        }
        catch (err) {
            logger.warn({ trace: err.stack }, 'failed to decline passkey_challenge, ignoring');
        }
    });
