...
export const makeSocket = (config) => {
    ...
    /**
     * The pairing code and the account identity it belongs to live on
     * authState.creds, and the pairing response decrypts against them long after
     * the registration IQ resolves -- the response only arrives once the user
     * types the code on their phone. A second request in that window overwrites
     * both, so the earlier response can no longer be decrypted and the identity
     * it pairs into is the wrong one. Refuse the second request instead.
     *
     * The guard outlives the code itself by PAIRING_RESPONSE_GRACE_MS: WhatsApp
     * rotates a pairing code every 3 minutes (observed on web), and a code typed
     * in its last seconds still has a response arriving after it expired.
     */
    let pendingPairing;
    ...
    /** Abandons a pending pairing attempt so a new code can be requested before it expires. */
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        ...
        const timer = setTimeout(() => {
            logger.info({ phoneNumber }, 'pairing code expired without being used');
            pendingPairing = undefined;
        }, guardMs);
        timer.unref?.();
        pendingPairing = { phoneNumber, expiresAt: Date.now() + pairingCodeTimeoutMs, timer };
        ...
        try {
                ...
                content: [
                    {
                        ...
                        content: [
                            ...
                            },
                            {
                                ...
                                // A single zero byte, not the character "0".
                                // WhatsApp Web sends new Uint8Array(1) here, which
                                // encodes as binary 0x00; the string encodes as 0x30
                                // and is a different value on the wire.
                                attrs: {},
            ...
            // The registration never left the socket, so no pairing response can
            // arrive for it. Holding the guard here would block a fresh attempt
            // for no reason.
            clearPendingPairing();
            throw error;
        }
