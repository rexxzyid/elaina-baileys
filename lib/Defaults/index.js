...
const version = [2, 3000, 1045855472];
...
export const DEFAULT_CACHE_TTLS = {
    ...
    OUTBOUND_RESEND: 60 // 1 minute
};
/** Upper bound on messages held for a store-free resend after a retryable nack. */
export const MAX_OUTBOUND_RESEND_ENTRIES = 512;
/**
 * Extra time the pairing guard is held past the code's own lifetime. A code
 * typed in its final seconds still has a response in flight afterwards, and
 * that response decrypts against the pairing state; releasing the guard the
 * instant the code expires would let a fresh request overwrite it mid-flight.
 */
