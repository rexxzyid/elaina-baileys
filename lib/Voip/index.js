...
export class VoipClient {
    constructor(config = {}) {
        ...
        this.call = async (phoneNumber, opts = {}) => {
            ...
            call._video = !!(opts.video ?? opts.isVideo);
...
this._handleVideoCaptureStart = config => {
    const call = this._activeCall;
    if (!call?._video) {
        this._log("[RTC VIDEO] capture diminta tapi call ini bukan video, diabaikan");
        return;
    }
    if (this._videoFeeder) {
        return;
    ...
    this._log("[RTC VIDEO] start feeder:", { width, height, fps, playlist });
    ...
    this._videoFeeder = new VideoFeeder(
        ...
        (frame, w, h, rate) => {
            if (this._engine && (!this._engine.isInitialized || this._engine.isInitialized())) {
                this._engine.sendVideoFrame(frame, w, h, rate, VIDEO_FORMAT_I420, 0);
            }
        },
        undefined,
        {
...
this._handleVideoCaptureStop = () => {
    this._log("[RTC VIDEO] stop feeder");
    try { this._videoFeeder?.stop(); } catch {}
    this._videoFeeder = null;
