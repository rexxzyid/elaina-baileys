...
class AIRich extends BaseBuilder {
	...
	async build(
		jid,
		{ bypassDownload = true, forwarded = true, forwardWrapper = false, notification = false, includesUnifiedResponse = true, includesSubmessages = true, botJid = AIRich.DEFAULT_BOT_JID, disclaimerText = '', quoted, quotedParticipant, messageId, ...options } = {}
	) {
	...
	async buildEdit(targetJid, targetId, { msg, messageId, forwardWrapper = false, ...options } = {}) {
		if (!msg) {
	...
	static wrapRichResponse(richResponseMessage, forwardWrapper = false) {
