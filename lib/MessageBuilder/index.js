...
class AIRich extends BaseBuilder {
	...
	async build(
		jid,
		{ bypassDownload = true, forwarded = true, forwardWrapper = true, notification = false, includesUnifiedResponse = true, includesSubmessages = true, botJid = AIRich.DEFAULT_BOT_JID, disclaimerText = '', quoted, quotedParticipant, messageId, ...options } = {}
	) {
			...
			jid,
			{
				messageContextInfo: {
					...
					botMetadata: {
						...
						...notif,
						verificationMetadata: this._signedVerificationMetadata ?? AIRich.generateVerificationMetadata(),
		...
		built.aiVerification = carriesMetaSignature ? 'preserved' : 'placeholder';
	...
	static _warnPlaceholderSignature(client) {
		...
		const message = 'AIRich: kartu ini dibangun sendiri tanpa tanda tangan Meta, jadi di klien yang mengaktifkan '
			+ 'ai_rich_response_unknown_sender_verification_masking_enabled hanya judulnya yang tampil. '
