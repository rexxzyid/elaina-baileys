...
const newIdFor = (client) => (typeof client?.newMessageId === 'function' ? client.newMessageId(client?.user?.id) : generateMessageIDV2(client?.user?.id));
...
class Button extends BaseBuilder {
	...
	async build(jid, { messageId, ...options } = {}) {
		...
		return generateWAMessageFromContent(
			...
			{ messageId: messageId || newIdFor(this.#client), ...options }
		);
	}
...
class AIRich extends BaseBuilder {
	...
	async sendEdit(jid, id, { msg, messageId, additionalNodes = [], ...options } = {}) {
		...
		const msgEdit = await this.buildEdit(jid, id, {
			msg,
			messageId: messageId || newIdFor(this.#client),
			...options,
		});
