...
export const embeddedTab = ({ id, header, tabHeader, sections = [] } = {}) => {
    if (!Array.isArray(sections)) {
        throw new TypeError('embeddedTab sections must be an array of sections')
    }
    return trimEmpty({ id: id ?? randomUUID(), header, tab_header: tabHeader, sections })
}
...
export const embeddedScreen = ({ id, title, content, tabs, header, body, artifacts, steps, stepEntries, sources, pollId } = {}) => {
