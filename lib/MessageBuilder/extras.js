...
export const sendHtmlApp = async (sock, jid, html, { title = '', label, trustedSources, height, typename, id, bypassDownload = false, guard = 'warn', ...options } = {}) => {
    if (!sock) {
...
export const embeddedScreen = ({ id, title = '', content = [], tabs, header, body } = {}) => {
    if (!Array.isArray(content)) {
        throw new TypeError('embeddedScreen content must be an array of sections')
    }
    if (tabs !== undefined && !Array.isArray(tabs)) {
        throw new TypeError('embeddedScreen tabs must be an array')
    }
    return trimEmpty({
        id: id ?? randomUUID(),
        title: String(title),
